...
function validate(values: FormValues): FieldErrors {
  ...
  if (!values.providerName.trim()) {
    errors.providerName = "Informe o nome da consultoria.";
  }

  if (!values.cnpj.trim()) {
    errors.cnpj = "Informe o CNPJ da consultoria.";
...
// Diferente de app/register/register-form.tsx: não há etapa de revisão
// (ver docs/sst-providers.md) — sucesso vai direto para /sst/dashboard, a
// sessão já está ativa (POST /api/sst/register seta o cookie via
// signUpEmailInternal).
export function SstRegisterForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      response = await fetch("/api/sst/register", {
        method: "POST",
  ...
  return (
    ...
    <form onSubmit={handleSubmit} noValidate method="post" className="grid gap-4">
      ...
      <div className="grid gap-2">
        <Label htmlFor="providerName">Nome da consultoria</Label>
        <div className="relative">
          ...
          <Input
            ...
            autoFocus
            placeholder="Minha Consultoria SST Ltda"
        ...
        {fieldErrors.providerName ? (
          <p className="text-sm text-destructive">{fieldErrors.providerName}</p>
        ) : null}
      </div>
